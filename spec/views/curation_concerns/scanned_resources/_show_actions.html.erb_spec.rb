...
RSpec.describe "curation_concerns/scanned_resources/_show_actions.html.erb" do
  ...
  before do
    ...
    render partial: "curation_concerns/scanned_resources/show_actions",
           locals: { collector: collector, editor: editor }
  end
  it "renders a reorder link" do
    expect(rendered).to have_link(
      I18n.t('file_manager.link_text'),
      href: file_manager_curation_concerns_scanned_resource_path(
        id: resource.id
      )
    )
  end
  it "renders an Edit Structure link" do
    expect(rendered).to have_link(
      "Edit Structure",
      href: structure_curation_concerns_scanned_resource_path(
        id: resource.id
      )
    )
  end
  ...
  context "when there's a parent presenter" do
    ...
    it "renders a link to the contextual file manager" do
      expect(rendered).to have_link(
        "File Manager",
        href: file_manager_curation_concerns_parent_scanned_resource_path(
          id: resource.id, parent_id: parent_presenter.id
        )
      )
    end
  end
