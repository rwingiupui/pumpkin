...
RSpec.describe ResourceIdentifier do
  ...
  describe "#to_s" do
    ...
    it "changes when reordered" do
      ...
      actor = CurationConcerns::CurationConcern \
              .actor(scanned_resource, FactoryGirl.build(:user))
      actor.update("ordered_member_ids" => [file_set2.id, file_set1.id])

      expect(resource_id.reload.to_s).not_to eq old_id
    end
