...
RSpec.describe UpdatesOCR do
  let(:resource) { described_class.new(FactoryGirl.build(:scanned_resource)) }
  let(:file_set) { FactoryGirl.create(:file_set, content: file) }
  let(:file) {
    File.open(Rails.root.join("spec", "fixtures", "files", "page18.tif"))
  }
  ...
  context "when ocr_language is changed" do
    ...
    def stub_language(language, content)
      allow(Processors::OCR).to receive(:encode) \
        .with(anything, { options: "-l #{language}" }, anything) do |*args|
